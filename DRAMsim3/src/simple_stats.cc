...
SimpleStats::SimpleStats(const Config& config, int channel_id)
    ...
    InitStat("num_drfmb_cmds", "counter", "Number of DRFMB commands"); // [DRFM] Number DRFMB commands
    InitStat("num_preab_cmds", "counter", "Number of PREAB commands");
    InitStat("num_presb_cmds", "counter", "Number of PRESB commands");
    ...
    InitStat("rfmab_energy", "double", "RFMAB energy"); // [RFM] RFMAB energy
    ...
    InitHistoStat("bursty_access_count", "Bursty access count", 0, 32, 32);
...
void SimpleStats::PrintFinalStats(bool stdout) {
    ...
    if (stdout) {
        const_cast<LRUCache*>(&config_.hydra_rcc)->print_stats();
    }
...
void SimpleStats::UpdateEpochStats() {
    ...
    doubles_["rfmab_energy"] =
        epoch_counters_["num_rfmab_cmds"] * config_.rfmab_energy_inc; // [RFM] RFMAB energy
    ...
    double total_energy = doubles_["act_energy"] + doubles_["read_energy"] +
                          ...
                          doubles_["refb_energy"] + background_energy + doubles_["rfmab_energy"]; // [RFM] RFMAB energy
