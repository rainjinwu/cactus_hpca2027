...
bool MemorySystem::IsInDRFM(uint64_t hex_addr) const {
    return dram_system_->IsInDRFM(hex_addr);
}

bool MemorySystem::IsInREF(uint64_t hex_addr) const {
    return dram_system_->IsInREF(hex_addr);
}
