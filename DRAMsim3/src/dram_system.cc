...
bool BaseDRAMSystem::IsInDRFM(uint64_t hex_addr) const {
    int channel = GetChannel(hex_addr);
    return ctrls_[channel]->IsInDRFM(hex_addr);
}

bool BaseDRAMSystem::IsInREF(uint64_t hex_addr) const {
    int channel = GetChannel(hex_addr);
    return ctrls_[channel]->IsInREF(hex_addr);
}
